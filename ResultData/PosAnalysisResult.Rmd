...
for(eval_pos_idx in DrawSpreads){
  ...
  posStepDays %>% group_by(days) %>%
    do(scene=createPositionEvalTable(position=thePosition,udlStepNum=udlStepNum,udlStepPct=udlStepPct,
                                     multi=PosMultip,hdd=as.numeric(ifelse(.$days+EvalFuncSetting$holdDays<=max(evaldays),
                                                                           as.numeric(ifelse(.$days<=EvalFuncSetting$holdDays,.$days,EvalFuncSetting$holdDays)),
                                                                           max(evaldays)-.$days)),
                                     HV_IV_Adjust_Ratio=HV_IV_Adjust_Ratio)) -> posStepDays
