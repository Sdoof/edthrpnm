...
VolatilitySensitivityChange=0.3

#DrawLineSizeMax
DrawLineSizeMax=1.4

#First day line mede Blank
FirstDayLineMadeBlank=FALSE
...
for(eval_pos_idx in DrawSpreads){
  ...
  if(VolSensitivityCheck){
    ...
    #volatility Plus
    posStepDays_vc %>% group_by(days) %>% rowwise() %>% do(days=.$days,scene2=adjustPosChg(.$scene,base_vol_chg=vol_chg,
    ...
    posStepDays_vc %>% group_by(days) %>% rowwise() %>% do(days=.$days,scene2=adjustPosChg(.$scene,base_vol_chg=(-1)*vol_chg,
                                                                                           multi=PosMultip,hdd=holdDays,
                                                                                           HV_IV_Adjust_Ratio=EvalFuncSetting$HV_IV_Adjust_Ratio,isDebug=FALSE)) -> tmp
    unlist(tmp$days) -> posStepDays_vc$days ; tmp$scene2 -> posStepDays_vc$scene ;rm(tmp)
