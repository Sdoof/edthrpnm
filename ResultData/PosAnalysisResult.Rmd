...
for(eval_pos_idx in DrawSpreads){
  ...
  if(VolSensitivityCheck){
    ...
    #volatility Plus
    posStepDays_vc %>% group_by(days) %>% rowwise() %>% do(days=.$days,scene2=adjustPosChg(.$scene,.$days-1,base_vol_chg=vol_chg,multi=PosMultip,hdd=holdDays,HV_IV_Adjust_Ratio=EvalFuncSetting$HV_IV_Adjust_Ratio,isDebug=FALSE)) -> tmp
    ...
    posStepDays_vc %>% group_by(days) %>% rowwise() %>% do(days=.$days,scene2=adjustPosChg(.$scene,.$days-1,base_vol_chg=(-1)*vol_chg,multi=PosMultip,hdd=holdDays,HV_IV_Adjust_Ratio=EvalFuncSetting$HV_IV_Adjust_Ratio,isDebug=FALSE)) -> tmp
    unlist(tmp$days) -> posStepDays_vc$days ; tmp$scene2 -> posStepDays_vc$scene ;rm(tmp)
  ...
  if(ShowDeltaHedge && VolSensitivityCheck){
    ...
    gg<-ggplot(drawGrktbl_DltHgd,aes(x=UDLY,y=profit,group=day))+
      ...
      geom_line(y=drawGrktbl_DltHgd_vc_plus$profit,colour="blue",size=draw_line_size,linetype=draw_line_type)+
      geom_line(y=drawGrktbl_DltHgd_vc_mnus$profit,colour="red",size=draw_line_size,linetype=draw_line_type)+
      geom_point(x=thePositonGrks$UDLY,y=0,size=4.0,colour="black")+
      #geom_line(y=drawGrktbl$ThetaEffect,colour="orange",size=draw_line_size,linetype=draw_line_type)+
      #geom_line(x=drawGrktbl$UDLY,y=drawGrktbl$DeltaEffect,colour="skyblue3",size=draw_line_size,group=drawGrktbl$day,linetype=draw_line_type)+
      #geom_point(x=thePositonGrks$UDLY,y=0,size=4.0,colour="black")+
