...
evalTargetIdx<-data.frame(spread_idx=1,scenario_idx=1,stim_idx=10)
...
gg <- ggplot(data.frame(days=seq(1,Stimdays,by=1),udly=UDLY_path),aes(x=days,y=udly))+geom_point()+geom_line()+
  geom_point(x=0,y=iniUDLY,size=3.0,pch=3)+xlim(0,Stimdays)+ylim(min(UDLY_path,iniUDLY),max(UDLY_path,iniUDLY))
print(gg)
```
...
gg <- ggplot(data.frame(days=seq(1,Stimdays,by=1),IVIDX=IVIDX_path),aes(x=days,y=IVIDX))+geom_point()+geom_line()+
  geom_point(x=0,y=iniIVIDX,size=3.0,pch=3)+xlim(0,Stimdays)+ylim(min(IVIDX_path,iniIVIDX),max(IVIDX_path,iniIVIDX))
print(gg)
```
...
gg <- ggplot(data.frame(days=seq(1,Stimdays,by=1),profit=Profit_path),aes(x=days,y=profit))+geom_point()+geom_line()+
  geom_point(x=0,y=0,size=3.0,pch=3)+xlim(0,Stimdays)+ylim(min(Profit_path,0),max(Profit_path,0))
print(gg)
```
...
```
###sigmoid (1-sigmoid(DeltaE+VegaE))/sigmoid(ThetaE+GammaE)
```{r echo = FALSE}
a<-0.0001
print(a)
