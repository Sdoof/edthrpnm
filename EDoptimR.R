...
pressure_vessel_B <-
  list(
    ...
    obj = function(x) {
      ...
      # Optionの売買最適化問題では、
      #   正の数の場合 floor() or trunc()
      #   負の数の場合 ceiling() or trunc()が良さそうだ
      #   
      #   あるいは、ATMに近くかつ 
      #       C1. -1.0 < x < -0.4  なら-1に丸める 
      #           (あるいは-1.0 < x < 0 ならfloor()を用いる)
      #       C2.  0 < x < 1.0 なら1に丸める
      #           (あるいは 0 < x < 1.0ならceiling()を用いる)
      #     のようなOptionのポジションに重みによって条件を変化させても良い
      #   
      # ( x1 <- seq(-2, 4, by = .5) )
